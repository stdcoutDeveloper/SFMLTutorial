...
namespace SFMLTutorial
{
	...
	public:
		...
		{
			body_rect_.setSize(sf::Vector2f(graphics_size_ - 1, graphics_size_ - 1));
			Reset();
		}
		...
		void Render(sf::RenderWindow& window)
		{
			...
			body_rect_.setPosition(head->position_.x * graphics_size_, head->position_.y * graphics_size_);
			...
			{
				body_rect_.setPosition(itr->position_.x * graphics_size_, itr->position_.y * graphics_size_);
				window.draw(body_rect_);
			}
		}
